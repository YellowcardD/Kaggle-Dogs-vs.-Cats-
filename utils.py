...
def preprocess_image_for_stacking(img):
    """
    deploy center crop for the subsequent stacking
    """
...
def perpare_train_images_from_file(path):
    """
    write the preprocessed image to specific path
    """
...
def data_augmentation_v1(path):
    """
    version 1 of data augmentation
    """
    cnt = 0
    for i in range(0, 12500):
        base_name = 'cat.{}.jpg'
...
def data_augmentation_v2(path):
    """
    version 2 of data augmenatation
    """
...
def VGG16(IMG_SIZE):
    ...
    model = keras.applications.VGG16(include_top=False, input_shape=(IMG_SIZE, IMG_SIZE, 3), weights='imagenet')
    output = GlobalAveragePooling2D()(model.output)
    output = Dense(1, activation='sigmoid')(output)
    model = Model(inputs=model.inputs, outputs=output)
    return model
