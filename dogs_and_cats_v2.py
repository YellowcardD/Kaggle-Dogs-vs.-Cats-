"""augmentation v1,ten-crop, resolution=250, model=inception_v3"""
...
def batch_generator(items, batch_size):
    """
    generate image(s) from file for given batch_size
    """
    minibatch = []
    cnt = 0
    for item in items:
...
def train_generator(files, labels):
    """
    generate iamges and labels for training stage
    """
    while True:
...
def get_ten_crop_imgs(img):
    """
    for a given image, we get an array of 10 crop images(lower left, upper left, lower right, upper right,
    central and same for its mirror image)
    """
    batch_imgs = []
...
def ten_crop_test_generator():
    """
    generate 10 crop images for each image
    """
    while True:
...
decay = ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=2)
# model.fit_generator(
...
result = np.clip(result, 0.005, 0.995)
